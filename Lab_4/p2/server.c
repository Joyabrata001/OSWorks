...
int dat[100]; /* for creating the array before-hand */
int dat_size; /* filled size of dat*/
void createArray(int *dat);
...
int main()
{
    ...
    while (solve->status != -1) /* while server has not terminated */
    {
        ...
        {
            if (solve->task_id == t_id - 1) /* means an array is already created while status was 2*/
            {
                solve->size = dat_size;
                memcpy(solve->data, dat, dat_size);
                solve->task_id = t_id;
            }
            else /* array not created from before, need to really create it*/
            {
                createArray(solve->data);
                solve->size = dat_size;
                solve->task_id = ++t_id;
                printf("\t Task ID = %d\n", solve->task_id);
            }
...
void createArray(int *dat)
{
    dat_size = rand() % (MAX_SIZE_OF_ARRAY - MIN_SIZE_OF_ARRAY + 1) + MIN_SIZE_OF_ARRAY;
    printf("\nUnsorted array: ");
